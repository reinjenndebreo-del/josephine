...
async function renderUsersCards() {
  try {
    const response = await fetch('../../api/admin/get/allUsers.php', {
      method: 'GET',
      credentials: 'include',
    });
...
async function renderOrdersCards(filterStatus = 'all') {
  try {
    ...
    const filteredOrders = filterStatus === 'all' 
      ? allOrdersData 
      : allOrdersData.filter(order => order.payment_status === filterStatus);
    ...
    filteredOrders.forEach((order) => {
      ...
      const card = document.createElement('div');
      card.className = 'bg-[#121212] border border-gray-700 rounded-lg p-5 hover:border-green-500 transition-all duration-300 hover:shadow-xl';
      card.innerHTML = `
        <div class="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
          <!-- Left Section: Customer & Product Info -->
          <div class="flex-1">
            <div class="flex items-start justify-between mb-3">
              <div>
                <h3 class="text-xl font-bold text-[#fffeee] mb-1">${order.user_name || 'N/A'}</h3>
                <div class="flex flex-wrap items-center gap-2 text-xs text-gray-400">
                  <span><i class="bi bi-hash text-green-500"></i> ${order.order_id}</span>
                  <span>•</span>
                  <span><i class="bi bi-credit-card-2-front text-purple-500"></i> ${order.payment_id || 'N/A'}</span>
                </div>
              </div>
              <button class="delete-order text-red-400 hover:text-red-600 transition-colors text-xl lg:hidden" data-id="${order.order_id}">
                <i class="bi bi-trash"></i>
              </button>
            </div>
            ...
            <div class="bg-[#1e1e1e] rounded-lg p-4 border border-gray-700">
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-2">
                  <i class="bi bi-box-seam text-green-500 text-xl"></i>
                  <span class="text-white font-semibold text-lg">${order.product_name || 'N/A'}</span>
                </div>
                <span class="text-[var(--color-primary-600)] font-bold text-xl">₱${parseFloat(order.price || 0).toFixed(2)}</span>
              </div>
              
              <div class="flex items-center gap-4 text-sm">
                <div class="flex items-center gap-1 text-gray-300 bg-[#121212] px-3 py-1 rounded-full">
                  <i class="bi bi-cart3 text-green-500"></i>
                  <span>Qty: <strong>${order.quantity}</strong></span>
                </div>
                ${order.customize && order.customize !== 'N/A' && order.customize !== '' ? `
                  <div class="flex items-center gap-1 text-gray-300 bg-[#121212] px-3 py-1 rounded-full">
              ...
              <div class="flex items-center gap-2 text-xs text-gray-400 mt-3 pt-3 border-t border-gray-700">
                ...
                <span>${new Date(order.order_date).toLocaleString('en-US', { 
                  month: 'short', 
                  day: 'numeric', 
                  year: 'numeric',
                  hour: '2-digit',
          ...
          <div class="flex lg:flex-col items-center lg:items-end gap-3">
            <span class="text-sm px-4 py-2 rounded-full ${statusColors[order.payment_status] || 'bg-gray-500'} text-white font-bold whitespace-nowrap shadow-lg">
              <i class="bi bi-circle-fill text-xs mr-1"></i>
              ${statusText[order.payment_status] || order.payment_status || 'Pending'}
            </span>
            <button class="delete-order text-red-400 hover:text-red-600 hover:bg-red-500 hover:bg-opacity-10 transition-all text-xl p-2 rounded-lg hidden lg:block" data-id="${order.order_id}">
              <i class="bi bi-trash"></i>
            </button>
          </div>
        </div>
...
async function renderPaymentsCards(filterStatus = 'all') {
  try {
    ...
    const filteredPayments = filterStatus === 'all' 
      ? allPaymentsData 
      : allPaymentsData.filter(payment => payment.payment_status === filterStatus);
    ...
    paymentsCount.textContent = filteredPayments.length;
    ...
    filteredPayments.forEach((payment) => {
      ...
      const card = document.createElement('div');
      card.className = 'bg-[#121212] border border-gray-700 rounded-lg p-5 hover:border-purple-500 transition-all duration-300 hover:shadow-xl';
      card.innerHTML = `
        <div class="flex items-start justify-between mb-4">
          <div class="flex-1">
            <h3 class="text-lg font-bold text-[#fffeee] mb-1">${payment.user_name || 'N/A'}</h3>
            <p class="text-xs text-gray-400"><i class="bi bi-hash text-purple-500"></i> ${payment.payment_id}</p>
          </div>
          <button class="delete-payment text-red-400 hover:text-red-600 hover:bg-red-500 hover:bg-opacity-10 transition-all text-lg p-2 rounded-lg" data-id="${payment.payment_id}">
            <i class="bi bi-trash"></i>
          </button>
        </div>
        
        <!-- Amount & Status -->
        <div class="bg-gradient-to-br from-purple-900 to-purple-800 rounded-lg p-4 mb-4 border border-purple-600">
          <div class="flex items-start justify-between mb-3">
            <div>
              <p class="text-xs text-purple-200 mb-1">Total Amount</p>
              <p class="text-3xl font-bold text-white">₱${parseFloat(payment.total_amount || 0).toFixed(2)}</p>
            </div>
            <span class="text-xs px-3 py-1.5 rounded-full ${statusColors[payment.payment_status] || 'bg-gray-500'} text-white font-bold shadow-lg">
              <i class="bi ${statusIcons[payment.payment_status] || 'bi-circle'} mr-1"></i>
              ${statusText[payment.payment_status] || payment.payment_status || 'Pending'}
            </span>
          </div>
        </div>
        ...
        <div class="space-y-2.5 mb-4">
          <div class="flex items-center gap-2 text-sm bg-[#1e1e1e] px-3 py-2 rounded-lg border border-gray-700">
            <i class="bi bi-credit-card-2-front text-purple-500"></i>
            <span class="text-gray-300">Ref: <strong class="text-white">${payment.reference_number || 'N/A'}</strong></span>
          </div>
          <div class="flex items-center gap-2 text-sm bg-[#1e1e1e] px-3 py-2 rounded-lg border border-gray-700">
            <i class="bi bi-wallet2 text-purple-500"></i>
            <span class="text-gray-300">${payment.mop || 'N/A'}</span>
          </div>
          <div class="flex items-center gap-2 text-xs text-gray-400 px-3 py-2">
            <i class="bi bi-calendar3 text-purple-500"></i>
            <span>${new Date(payment.payment_date).toLocaleString('en-US', { 
              month: 'short', 
              day: 'numeric', 
              year: 'numeric',
              hour: '2-digit',
        ...
        ${payment.screenshot_path ? `
          <button class="show-screenshot w-full bg-purple-600 hover:bg-purple-700 text-white py-2.5 rounded-lg mb-3 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl" data-src="${payment.screenshot_path}">
            <i class="bi bi-image-fill"></i> View Proof
          </button>
        ` : `
          <div class="w-full bg-gray-800 text-gray-500 py-2.5 rounded-lg mb-3 text-center text-sm border border-gray-700">
            <i class="bi bi-image"></i> No proof uploaded
          </div>
        `}

        <div class="flex items-center gap-2">
          <select class="payment-status flex-1 bg-[#1e1e1e] border border-gray-600 rounded-lg px-3 py-2.5 text-white text-sm focus:border-purple-500 focus:outline-none" data-id="${payment.payment_id}">
          ...
          <button class="save-status hidden bg-green-500 hover:bg-green-600 text-white px-4 py-2.5 rounded-lg transition-all duration-300 font-semibold shadow-lg" data-id="${payment.payment_id}">
            <i class="bi bi-check2-circle"></i>
          </button>
        </div>
      `;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const orders = await renderOrdersCards('pending'); // Load pending orders by default
  const payments = await renderPaymentsCards('pending'); // Load pending payments by default
  await renderDeliveriesCards();

  // Update dashboard stats (use full data for stats, not filtered)
  updateDashboardStats(users || [], reservations || [], allOrdersData || [], allPaymentsData || []);

  // Add order filter listener
  const orderFilter = document.getElementById('order-filter');
  if (orderFilter) {
    orderFilter.addEventListener('change', (e) => {
      renderOrdersCards(e.target.value);
    });
  }
  ...
  if (paymentFilter) {
    paymentFilter.addEventListener('change', (e) => {
      renderPaymentsCards(e.target.value);
    });
  }
});
